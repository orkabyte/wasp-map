...
;(function (factory) {
	...
	L.Heatmap = L.GridLayer.extend({
		...
		fetchData: function (npcNames, npcIds, range) {
			this.getIds(npcNames, npcIds).then((ids) => {
				Promise.allSettled(ids.map((id) => fetch(`${this.options.folder}/npcids/npcid=${id}.json`)))
					...
					.then((data) =>
						data.length !== 0 ? data : Promise.reject(new Error("Unable to find any npcids."))
					)
					.then((data) => data.flat())
					...
					.then((npcs) => {
						...
						Promise.allSettled(
							keys.map((key) => fetch(`${this.options.folder}/collisions/-1/${key}.json`))
						)
							.then((responses) =>
								Promise.all(
									responses
		...
		constructDataCache: function (mapData, keys, npcs) {
			...
			if (this.options.showHeat) {
				...
				this._maxHeat = this._eachMaxHeat.length
					? Math.max.apply(null, this._eachMaxHeat)
					: undefined
			}
		},
		...
		array: {
			...
			add: function (arrays) {
				...
				return this.starMap(newArray, (_, i, j) =>
					arrays.map((array) => array[i][j]).reduce((a, b) => a + b, 0)
				)
			},

			//maps function fn over a 2d array, returning the resulting array
			starMap: function (array, fn) {
				return array.map((subarray, index, array) =>
					subarray.map((value, jndex) => fn(value, index, jndex, array))
				)
			},
			...
			starEach: function (array, fn) {
				return array.forEach((subarray, index, array) =>
		...
		getColor: function (tileData) {
			...
			if (!this.colors[key]) {
				this.colors[key] =
					"rgba(" +
					parseInt((255 * tileData) / this._maxHeat) +
					",0, 0, " +
					parseInt((100 * tileData) / this._maxHeat) / 100 +
					")"
			}
		...
		getTextColor: function (tileData) {
			...
			if (!this.textColors[key]) {
				...
				this.textColors[key] =
					"rgba( 255 ,255, 255, " + parseInt((100 * tileData) / this._maxHeat) / 100 + ")"
			}
		...
		getFeature: function (npc) {
			...
			npc.feature = this._collisionData[key]
				? this._collisionData[key][npc.x & (this.options.gridSize - 1)][
						npc.y & (this.options.gridSize - 1)
					].f
				: null
		},
		...
		getRange: function (npc, range) {
			...
			let allKeys = this.array
				.combinations(plane, radiusX, radiusY)
				.map((tile) => this._generateDataKey(tile))
		...
		_generateDataKey: function (...args) {
			...
			if (typeof args[0] !== "object") {
				...
				return [
					args[0].p,
					args[0].x >> this.options.bitShift,
					args[0].y >> this.options.bitShift
				].join("_")
			}
		},
	...
	L.Teleports = L.DynamicIcons.extend({
		...
		parseData: function (data, watery) {
			let dataCollection = this.parseSheet(data)
				...
				.map((item) => {
					...
					if ("destination" in item) {
						item.destination.plane = watery[item.destination.x >> 6][
							item.destination.y >> 6
						].includes((item.destination.x << 14) + item.destination.y)
			...
			let teleports = dataCollection.filter(
			...
			all_icons.forEach(
				(item) => (item.watery = watery[item.x >> 6][item.y >> 6].includes((item.x << 14) + item.y))
			)
		...
		getIconUrl: function (item) {
			...
			if (filename) {
				...
				JSON.stringify(item).includes("agility") ||
				JSON.stringify(item).includes("Agility")
			) {
				//shortcut icon
		...
		fanOut: function (original_marker) {
			...
			let affectedIcons = this._icons[key].icons.filter((marker) =>
			...
			affectedIcons.forEach((marker, index) =>
				this.fan(polygonCenter, marker, polygonPoints[index])
			)

			let eventFn = (e) =>
				this.checkUnFan(e, polygonCenter, radius, eventFn, () => this.unFanAll(affectedIcons))
		...
		createIcon: function (item) {
			...
			if (item.iconUrl) {
				...
				destinationMarker = L.marker(
					[item.y + 0.4 + 0.2 * Math.random(), item.x + 0.4 + 0.2 * Math.random()],
					{
						icon: thisIcon,
						alt: item.name,
						riseOnHover: true
	...
	L.CustomParseTeleports = L.Teleports.extend({
				...
				const dataPromise = fetch(
					...
					response.ok
						...
						: response
								.json()
								.then((e) =>
									Promise.reject(new Error(e.error.message)).then(() => {}, console.error)
								)
				)
		...
		createIcon: function (item) {
			...
			L.MarkerIcon.bindSelection(destinationMarker)
	...
	L.CrowdSourceMovement = L.DynamicIcons.extend({
		...
		createNavigator: function (mode, map, item) {
			...
			newButton.onclick = function () {
				console.info(
					"navigating to",
					item.to_coordinate.p,
					item.to_coordinate.x,
					item.to_coordinate.y
				)
